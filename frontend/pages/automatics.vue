<script setup lang="ts">
		...
		[
			...
			},
			{
				...
				click: () => {
					...
					state.name = row.name;
					state.deleteVendorData = row.deleteVendorData;
				},
			},
			// {
			// 	label: "Удалить",
	...
	const schema = object({
		...
		deleteVendorData: array().required(),
	});

	type Schema = InferType<typeof schema>;

	const state: {
		deleteVendorData: string[];
		[key: string]: any;
	} = reactive({
		path_file: "",
		deleteVendorData: [],
	...
	async function onSubmit(event: FormSubmitEvent<Schema>) {
		const response = await $fetch("/api/automatics", {
			method: "POST",
			body: event.data,
		});
	...
	const actions = [
		{
			key: "deleteVendor",
			label: "Удалить товар с определенным продавцом",
			description: "Имена продавцов, с которыми не нужно конкурировать",
		},
	];
	...
	<div class="col-span-2 divide-y">
		...
		<div class="p-5">
			...
			>
				...
				<UFormGroup
					label="Что делать на странице товара:"
					name="deleteVendor_word"
				>
					<UTabs :items="actions" class="w-full">
						<template #item="{ item }">
							<UCard>
								<template #header>
									<p
								...
								>
									...
									>
										<UInput
											v-model="state.deleteVendor_word"
											@keyup.ctrl.enter="
												if (
													state.deleteVendor_word !=
													''
										...
										<UBadge
											...
											color="white"
											variant="solid"
											v-for="item in state.deleteVendorData"
											@click="
												state.deleteVendorData =
													state.deleteVendorData.filter(
														(value) =>
															value !== item
													)
											"
