...
export default class AutomaticsController {
  /**
   * Display a list of resource
   */
  async index({ inertia }: HttpContext) {
    const command = (await Command.findManyBy('type', 'automatics')).toReversed()

    return inertia.render('automatics', { commands: command })
  }
  ...
  async store({ request, inertia }: HttpContext) {
    ...
    const command = `node ${__dirname} automatics ${path_file} [${words.join(',')}] ${data.name}`
